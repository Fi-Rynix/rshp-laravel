...
<el-dialog>
    <dialog id="modalEdit" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            ...
            <el-dialog-panel
                ...
                <form action="{{ route('Dokter.RekamMedis.update-detail', $detail->iddetail_rekam_medis) }}" method="POST" class="space-y-3">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="flex-1">
                            <select name="idkode_tindakan_terapi" class="w-full rounded-md border @error('idkode_tindakan_terapi') border-red-500 @else border-slate-300 @enderror p-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                ...
                                @foreach ($kode_tindakan as $kode)
                                    <option value="{{ $kode->idkode_tindakan_terapi }}" {{ old('idkode_tindakan_terapi', $detail->idkode_tindakan_terapi) == $kode->idkode_tindakan_terapi ? 'selected' : '' }}>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <textarea name="detail" rows="4" class="w-full rounded-md border @error('detail') border-red-500 @else border-slate-300 @enderror p-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Jelaskan detail tindakan yang dilakukan..." required>{{ old('detail', $detail->detail) }}</textarea>
                            @error('detail')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
