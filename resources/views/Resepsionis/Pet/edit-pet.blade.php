...
<el-dialog>
    <dialog id="modalEdit-{{ $pet->idpet }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <h2 class="text-xl font-semibold mb-4">Edit Pet</h2>
                ...
                <form action="{{ route('Resepsionis.Pet.update-pet', $pet->idpet) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        class="w-full rounded-md border p-2 transition @error('idras_hewan') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">
                            ...
                            @foreach($rashevanlist as $rashewan)
                                <option value="{{ $rashewan->idras_hewan }}" {{ old('idras_hewan', $pet->idras_hewan) == $rashewan->idras_hewan ? 'selected' : '' }}>
                                    {{ $rashewan->nama_ras ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button"
                                command="close"
                                commandfor="modalEdit-{{ $pet->idpet }}"
