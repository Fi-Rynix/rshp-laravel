    ...
    <dialog id="modalCreate" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <h2 class="text-xl font-semibold mb-4">Tambah Pemilik</h2>
                ...
                <form action="{{ route('Resepsionis.Pemilik.store-pemilik') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="no_wa" value="{{ old('no_wa') }}"
                            class="w-full rounded-md border p-2 transition @error('no_wa') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">
                        @error('no_wa')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
