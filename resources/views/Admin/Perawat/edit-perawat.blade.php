        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <form action="{{ route('Admin.Perawat.save-perawat', $perawat->iduser) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label class="block mb-1 text-sm text-slate-700 font-medium">Pendidikan</label>
                        <input type="text" name="pendidikan" value="{{ old('pendidikan', $perawat->pendidikan) }}"
                        class="w-full rounded-md border p-2 transition @error('pendidikan') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">
                        @error('pendidikan')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="no_hp" value="{{ old('no_hp', $perawat->no_hp) }}"
                    ...
                    <div class="mb-4">
                        ...
                        class="w-full rounded-md border p-2 h-24 transition @error('alamat') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">{{ old('alamat', $perawat->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
