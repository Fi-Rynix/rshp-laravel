    ...
    <div class="p-6 border-b border-slate-200 bg-slate-50">
        <div class="flex gap-4 flex-wrap">
            ...
            <div class="flex-1 min-w-64">
                ...
                <form method="GET" action="{{ route('Admin.TemuDokter.daftar-temu-dokter') }}" class="flex gap-2">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($temuDokterlist as $row)
                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 font-medium text-slate-800">{{ $row->pet->nama ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $row->pet->pemilik->user->nama ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $row->roleUser->user->nama ?? '-' }}</td>
                        <td class="px-6 py-4">
                            @if($row->status === 'W')
...
@include('Admin.TemuDokter.create-temu-dokter')
@include('Admin.TemuDokter.cancel-temu-dokter')
