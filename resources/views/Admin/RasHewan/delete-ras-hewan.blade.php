<el-dialog>
    <dialog id="modalDelete-{{ $ras->idras_hewan }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
                ...
                <p class="text-gray-600 text-sm mb-6">
                    Apakah kamu yakin ingin menghapus ras
                    <span class="font-semibold text-gray-800">{{ $ras->nama_ras }}</span>?
                ...
                <div class="flex justify-end gap-2">
                    ...
                    {{-- Hapus --}}
                    <form action="{{ route('Admin.RasHewan.delete-ras-hewan', $ras->idras_hewan) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
