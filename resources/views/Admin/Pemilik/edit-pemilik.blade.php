...
<el-dialog>
    <dialog id="modalEdit-{{ $row->idpemilik }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <h2 class="text-xl font-semibold mb-4">Edit Pemilik</h2>
                ...
                <form action="{{ route('Admin.Pemilik.update-pemilik', $row->idpemilik) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="no_wa" value="{{ old('no_wa', $row->no_wa) }}"
                    ...
                    <div class="mb-4">
                        ...
                        class="w-full rounded-md border p-2 h-24 @error('alamat') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror">{{ old('alamat', $row->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
