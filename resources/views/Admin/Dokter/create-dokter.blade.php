    ...
    <dialog id="modalCreate" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <form action="{{ route('Admin.Dokter.store-dokter') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="bidang_dokter" value="{{ old('bidang_dokter') }}"
                            ...
                            placeholder="Contoh: Umum, Gigi, Bedah">
                        @error('bidang_dokter')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="alamat"
                            class="w-full rounded-md border p-2 h-24 transition @error('alamat') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
