<el-dialog>
    <dialog id="modalDelete-{{ $dokter->iddokter }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
                ...
                <p class="text-gray-600 text-sm mb-6">
                    Apakah kamu yakin ingin menghapus dokter
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button type="button"
                            command="close"
                            commandfor="modalDelete-{{ $dokter->iddokter }}"
                            class="px-4 py-2 bg-gray-200 text-gray-700 text-sm rounded-md hover:bg-gray-300">
                        Batal
                    </button>

                    {{-- Hapus --}}
                    <form action="{{ route('Admin.Dokter.delete-dokter', $dokter->iddokter) }}" method="POST">
                        @csrf
                        @method('DELETE')
