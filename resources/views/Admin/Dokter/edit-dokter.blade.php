...
<el-dialog>
    <dialog id="modalEdit-{{ $row->iddokter }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <form action="{{ route('Admin.Dokter.update-dokter', $row->iddokter) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-4">
                        <label class="block mb-1 text-sm text-slate-700 font-medium">Nama</label>
                        <input type="text" name="nama" value="{{ old('nama', $row->user->nama) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" value="{{ old('email', $row->user->email) }}"
                    ...
                    <div class="mb-4">
                        <label class="block mb-1 text-sm text-slate-700 font-medium">Bidang Dokter</label>
                        <input type="text" name="bidang_dokter" value="{{ old('bidang_dokter', $row->bidang_dokter) }}"
                        class="w-full rounded-md border p-2 transition @error('bidang_dokter') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">
                        @error('bidang_dokter')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block mb-1 text-sm text-slate-700 font-medium">No HP</label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $row->no_hp) }}"
                        class="w-full rounded-md border p-2 transition @error('no_hp') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">
                        @error('no_hp')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        class="w-full rounded-md border p-2 h-24 transition @error('alamat') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">{{ old('alamat', $row->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button"
                                command="close"
                                commandfor="modalEdit-{{ $row->iddokter }}"
