<el-dialog>
    <dialog id="modalEdit-{{ $dokter->iduser }}" class="fixed inset-0 z-[9000] bg-transparent backdrop:bg-transparent">
        ...
        <div class="fixed inset-0 z-[9500] flex items-center justify-center p-4">
            <el-dialog-panel class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                ...
                <h2 class="text-xl font-semibold mb-4">{{ $dokter->iddokter ? 'Edit' : 'Isi' }} Data Dokter</h2>
                ...
                <form action="{{ route('Admin.Dokter.save-dokter', $dokter->iduser) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="bidang_dokter" value="{{ old('bidang_dokter', $dokter->bidang_dokter) }}"
                    ...
                    <div class="mb-4">
                        ...
                        class="w-full rounded-md border p-2 h-24 transition @error('alamat') border-red-500 focus:ring-red-500 @else border-slate-300 focus:ring-blue-500 @enderror focus:border-transparent focus:ring-2">{{ old('alamat', $dokter->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button"
                                command="close"
                                commandfor="modalEdit-{{ $dokter->iduser }}"
