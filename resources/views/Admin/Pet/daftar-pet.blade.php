            ...
            <tbody class="divide-y divide-slate-200">
                @foreach($petlist as $row)
                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">{{ $row->pemilik->user->nama ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $row->rasHewan->nama_ras ?? '-' }}</td>
                    ...
                    @include('Admin.Pet.edit-pet', ['pet' => $row])
                    @include('Admin.Pet.delete-pet', ['pet' => $row])
